...
Web検索で最新情報を調べ、旅行やホテル滞在に役立つお役立ち情報5選を紹介する記事を作成してください。
  ...
  "tips": [
    {
      "number": 1,
      "heading": "見出し（簡潔に）",
      "body": "本文（200文字程度。具体的で実用的に）",
      "point": "ワンポイントアドバイス（100文字程度）"
    }
  ],
  "summary": "締めの一言（100文字程度）"
}

- tips は必ず number 1〜5 の5項目にすること。
- 旅行者・ホテル利用者が「明日から使える」具体的で実用的な情報にすること。
...
// 構造化データ(JSON-LD)：Article + お役立ち情報の ItemList
function buildJsonLd(article, date, pageUrl, imageUrl, description) {
  const tips = (article.tips || []).slice().sort((a, b) => a.number - b.number);
  const graph = {
...
function buildArticleHtml(article, imageRelPath, date) {
  const cards = (article.tips || [])
    .slice()
    .sort((a, b) => a.number - b.number)
    .map((t) => {
      // 1番目のカードには、その記事用に生成したヒーロー画像を表示
      const tipImage =
        t.number === 1
          ? `
          <img class="tip-image" src="${escapeHtml(imageRelPath)}" alt="${escapeHtml(
              t.heading
            )}" loading="lazy" />`
          : "";
      return `
      <article class="tip-card">
        <div class="tip-number">${escapeHtml(String(t.number))}</div>
        <div class="tip-body">${tipImage}
          <h2 class="tip-heading">${escapeHtml(t.heading)}</h2>
          <p class="tip-text">${escapeHtml(t.body)}</p>
          <p class="tip-point"><strong>ワンポイント：</strong>${escapeHtml(t.point)}</p>
        </div>
      </article>`;
    });

  // 1番目のカードの直後に自社アプリのプロモカードを挿入
  if (cards.length >= 1) {
    cards.splice(1, 0, buildPromoCard());
  }
  const tips = cards.join("\n");

  const description = toMetaDescription(
    article.summary ||
      (article.tips || []).map((t) => t.heading).join("、")
  );
